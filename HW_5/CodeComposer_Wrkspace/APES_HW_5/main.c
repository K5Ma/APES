 ...
 *       FreeRTOS mechanism.
 *
 * 7- [COMPLETED] The logger task should receive the messages from both the above tasks and log these values to
 ...
 *
 * 11- [COMPLETED] BONUS: Create an alert task which would be notified only when temperature goes above some
...
int main()
{
	...
	if(Alert_TaskInit())
	{
		Log_UART(Main, "ERROR", "Unable to init Alert task x_x");
	}
