...
void Logger_Task(void *pvParameters)
{
	...
	while(1)
	{
		if( xQueue_Logger != 0 )
		{
			...
			{
				switch(RXMessage.Source)
				{
					case Temp:
						snprintf(Temp_str, 100,  "%s%f", RXMessage.Msg, RXMessage.Value);
						break;

					case LED:
						snprintf(Temp_str, 100,  "%s - No. of LED toggles: %d", RXMessage.Msg, (uint16_t)RXMessage.Value);
						break;

					case Alert:
						snprintf(Temp_str, 100,  "%s", RXMessage.Msg);
						break;
				...
				Log_UART(RXMessage.Source, "INFO", Temp_str);
			}
		}
	}
